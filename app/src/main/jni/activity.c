...
// provides bindings between the activity APIs and a native implementation (lifecycle, input events, etc)
#include <android_native_app_glue.h>

// Define logging macros for usability
// __VA_ARGS__ - variable number of arguments (variadic)
// When the variadic macro is invoked, all the tokens in its argument list after the last named argument
// (this macro has none), including any commas, become the variable argument, which is stored in __VA_ARGS__
...
static void render_frame(struct driver *driver) {
    ...
    // calculate colors depending on touch events
...
static void handle_cmd(struct android_app *app, int32_t cmd) {
    // extract driver from app, which we saved earlier
    // TODO example of this extraction
...
* This means that whatever happens inside this method is
* not synchronized with the rest of the life-cycle
* events in the activity
*/
// !!! Your code has to keep this method active unless the activity should be destroyed !!!
void android_main(struct android_app *state) {
    LOGD("activity.c","android_main() - start");
    ...
    //Define app event handlers (hook our own callbacks)
    // Should reference a pointer where life-cycle events can be triggered (pause, resume, save instance state ...)
